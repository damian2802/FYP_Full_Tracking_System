...
if(isset($_POST['editname'])){
  ...
  $update = mysqli_query($conn, "UPDATE supervisor SET svName = '$nameChange' WHERE username='$activeuser'");

  if($update){
    $_SESSION['name'] = $nameChange;
    echo '<script language="javascript">';
    echo 'alert("Successfully Updated Name!"); location.href="sv_profile.php"';
    echo '</script>';
  }else{
    echo '<script language="javascript">';
    echo 'alert("Error Updating!"); location.href="sv_profile.php"';
    echo '</script>';
  }
  ...
  $select = mysqli_query($conn, "SELECT * FROM supervisor WHERE username='$activeuser'");
  $row = mysqli_fetch_array($select);

  if($newPassword == $newConfirmPassword){
    if(password_verify($oldPassword, $row['password'])){
      $hashedPassword = password_hash($newPassword, PASSWORD_DEFAULT);
      $update = mysqli_query($conn, "UPDATE supervisor SET password ='$hashedPassword' WHERE ='$activeuser'");
      if($update){
        echo '<script language="javascript">';
...
<html lang="en">
  ...
  <body>
    ...
    <!-- Content -->
    <div class="container-fluid">
        <h2 class="mt-3 py-3 px-5 text-left">Supervisor Profile</h2>
